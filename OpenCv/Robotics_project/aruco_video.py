...
from aruco_detections import *
#import arm_controls as arm_controls
 #control braccio
import keyboard
...
def main():
    ...
    url = 'http://192.168.0.122:8080/videofeed'
    ...
    while(True):
        ...
        x = centerCorner[0][1]
        y = centerCorner[0][0]
        converted_x = x*25/300
        converted_y = y*25/300
        transpolated_y = converted_y+11.5
        if converted_x>11.5:
            transpolated_x=converted_x-11.5
        else:
            transpolated_x=-1*(11.5-converted_x)
        encoded_data = str(transpolated_x)+","+str(transpolated_y)+"\r"
        if keyboard.is_pressed('m'):
            print(encoded_data)
            arm.write(encoded_data.encode())
